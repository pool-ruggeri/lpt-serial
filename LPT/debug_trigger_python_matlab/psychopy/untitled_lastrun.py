...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\pruggeri\\Desktop\\debug_trigger\\psychopy\\untitled_lastrun.py',
...
p_port = parallel.ParallelPort(address='0x3FE8')
text = visual.TextStim(win=win, name='text',
    ...
    languageStyle='LTR',
    depth=-1.0);
p_port_2 = parallel.ParallelPort(address='0x3FE8')
text_2 = visual.TextStim(win=win, name='text_2',
    ...
    languageStyle='LTR',
    depth=-3.0);
p_port_3 = parallel.ParallelPort(address='0x3FE8')
...
trialComponents = [p_port, text, p_port_2, text_2, p_port_3, text_3]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if p_port.status == STARTED:
        if frameN >= (p_port.frameNStart + 1.0):
    ...
    # *p_port_2* updates
    if p_port_2.status == NOT_STARTED and t >= 1-frameTolerance:
        # keep track of start time/frame for later
        ...
        win.callOnFlip(p_port_2.setData, int(11))
    if p_port_2.status == STARTED:
        if frameN >= (p_port_2.frameNStart + 1.0):
            # keep track of stop time/frame for later
    ...
    # *p_port_3* updates
    if p_port_3.status == NOT_STARTED and t >= 6-frameTolerance:
        # keep track of start time/frame for later
        ...
        win.callOnFlip(p_port_3.setData, int(12))
    if p_port_3.status == STARTED:
        if frameN >= (p_port_3.frameNStart + 1.0):
            # keep track of stop time/frame for later
...
if p_port_2.status == STARTED:
    win.callOnFlip(p_port_2.setData, int(0))
thisExp.addData('p_port_2.started', p_port_2.tStart)
thisExp.addData('p_port_2.stopped', p_port_2.tStop)
